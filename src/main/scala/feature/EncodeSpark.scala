...
object EncodeSpark {
  ...
  /** 对数据源进行one-hot编码
    ...
    */
  def oneHotEncode(resource: DataFrame, fMap: Broadcast[scala.collection.Map[String, Long]]) = {
  ...
  def main(args: Array[String]) {
    ...
    val sparkConf = new SparkConf().setAppName("EncodeSpark")
    ...
    val encodeDF = filter(oneHotEncode(bucketedDF, fMap))
    encodeDF.cache()
